...
export const UPDATE_ENGINE_REVISION = 4;
...
export function beginUpdate(state, runId = crypto.randomUUID()) {
  ...
  state.activeUpdate = {
    ...
    engineRevision: UPDATE_ENGINE_REVISION,
    attempt: 0,
    stagedPlan: null,
    appliedOperations: [],
...
export function recoverExhaustedUpdate(state, runId = crypto.randomUUID()) {
  ...
  const engineUpgraded = (exhausted.engineRevision || 0) < UPDATE_ENGINE_REVISION;
  if (!laterEvidence && !engineUpgraded) return null;
  state.activeUpdate = null;
  ...
  recovered.recoveredExhaustion = {
    ...
    reason: laterEvidence ? 'later-evidence' : 'engine-upgrade',
    resolvedAt: null
  };
  return recovered;
}
...
export function storePendingPlan(state, runId, plan, submission, review, {
  ...
  const satisfied = satisfiedStatusIntents.map(intent => ({
    ...
    unbindOnVerification: Boolean(intent.unbindOnVerification)
  }));
  ...
  if (!submission.operations.length) {
    ...
    for (const intent of satisfied) {
      ...
      if (!intent.unbindOnVerification && binding?.statusIntent?.revision === intent.revision &&
          binding.statusIntent.targetStatus === intent.targetStatus) {
